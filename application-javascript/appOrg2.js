...
//         ===> from directory test-network
//         ./network.sh deployCC -ccn events -ccp ../asset-transfer-events/chaincode-javacript/ -ccl javascript -ccep "OR('Org2MSP.peer','Org2MSP.peer')"
//
...
async function initGatewayForOrg2(useCommitEvents) {
	...
	const walletPathOrg2 = path.join(__dirname, 'walletOrg2', 'org2');
...
async function main() {
	...
	try {
		...
		try {
			...
			const contract1Org2 = network1Org2.getContract(chaincodeName);
			if(process.argv[3]) {
				assetKey = process.argv[3]

				try {
					// R E A D
					console.log(`${GREEN}--> Evaluate: GetPlace, - ${assetKey}`);
					const resultBuffer = await contract1Org2.evaluateTransaction('GetPlace', assetKey);
			...
			try {
				...
				listener = async (event) => {
					...
					if(event.eventName == "NewNet") {
						console.log("domains: ", asset.domains)
						if(asset.domains[org2]["status"] == "NotAccepted") {
							try {
								// C R E A T E
								console.log(`${GREEN}--> Submit Transaction: AcceptNet, ${asset.id}`);
								transaction = contract1Org2.createTransaction('AcceptNet');
								const resultBuffer = await transaction.submit(asset.id);
								const acceptedNet = JSON.parse(resultBuffer.toString('utf8'));
								console.log(acceptedNet);
								console.log(`${GREEN}<-- Submit AcceptNet Result: committed, asset ${asset.id}${RESET}`);
							} catch (createError) {
				...
				await contract1Org2.addContractListener(listener);
			} catch (eventError) {
			...
			try {
				...
				transaction = contract1Org2.createTransaction('CreatePlace');
			...
			try {
				...
				transaction = contract1Org2.createTransaction('CreateTransition');
			...
			/*try {
				...
				transaction = contract1Org2.createTransaction('UpdateAsset');
			...
			try {
				...
				const resultBuffer = await contract1Org2.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(org2, resultBuffer, 'blue', '10', 'Sam', '200');
			} catch (readError) {
			...
			try {
				...
				transaction = contract1Org2.createTransaction('TransferAsset');
			...
			try {
				...
				const resultBuffer = await contract1Org2.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(org2, resultBuffer, 'blue', '10', 'Mary', '200');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				...
				transaction = contract1Org2.createTransaction('DeleteAsset');
				...
				if (deleteError.toString().includes('ENDORSEMENT_POLICY_FAILURE')) {
					console.log(`${RED}Be sure that chaincode was deployed with the endorsement policy "OR('Org2MSP.peer','Org2MSP.peer')"${RESET}`);
				}
			}
			try {
				// R E A D
			...
			contract1Org2.removeContractListener(listener);

			//
			//  - - - - - -  B L O C K  E V E N T S  with  P R I V A T E  D A T A
			//
			console.log(`${BLUE} **** BLOCK EVENTS with PRIVATE DATA ****${RESET}`);
			const network2Org2 = await gateway2Org2.getNetwork(channelName);
			const contract2Org2 = network2Org2.getContract(chaincodeName);
			...
			try {
				...
				// now start the client side event service and register the listener
				console.log(`${GREEN}--> Start private data block event stream to peer in Org2${RESET}`);
				await network2Org2.addBlockListener(listener, {type: 'private'});
			} catch (eventError) {
			...
			try {
				...
				transaction = contract2Org2.createTransaction('CreateAsset');
			...
			try {
				...
				const resultBuffer = await contract2Org2.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(org2, resultBuffer, 'blue', '10', 'Sam', '100', '90');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// U P D A T E
				console.log(`${GREEN}--> Submit Transaction: UpdateAsset ${assetKey} update appraised value to 200`);
				transaction = contract2Org2.createTransaction('UpdateAsset');
			...
			try {
				...
				console.log(`${GREEN}--> Evaluate: ReadAsset, - ${assetKey} should now have appraised value of 200${RESET}`);
				const resultBuffer = await contract2Org2.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(org2, resultBuffer, 'blue', '10', 'Sam', '200', '90');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// T R A N S F E R
				console.log(`${GREEN}--> Submit Transaction: TransferAsset ${assetKey} to Mary`);
				transaction = contract2Org2.createTransaction('TransferAsset');
			...
			try {
				...
				console.log(`${GREEN}--> Evaluate: ReadAsset, - ${assetKey} should now be owned by Mary${RESET}`);
				const resultBuffer = await contract2Org2.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(org2, resultBuffer, 'blue', '10', 'Mary', '200', '180');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// D E L E T E
				console.log(`${GREEN}--> Submit Transaction: DeleteAsset ${assetKey}`);
				transaction = contract2Org2.createTransaction('DeleteAsset');
				await transaction.submit(assetKey);
				console.log(`${GREEN}<-- Submit DeleteAsset Result: committed, asset ${assetKey}${RESET}`);
			} catch (deleteError) {
				console.log(`${RED}<-- Failed: DeleteAsset - ${deleteError}${RESET}`);
			...
			// all done with this listener
			network2Org2.removeBlockListener(listener);*/
