...
async function main() {
	...
	try {
		...
		try {
			...
			setInterval(async () => {
				try {
					console.log(`${GREEN}--> Submit Transaction: GetMyTokens`);
					const transaction = contract1Org.createTransaction('GetMyTokens');
					...
					console.log(`${GREEN}<-- Submit GetMyTokens Result: ${tokens}${RESET}`);
				} catch (createError) {
